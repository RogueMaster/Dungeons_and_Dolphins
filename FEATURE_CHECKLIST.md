...
| Requested capability | Version 3.2.7 status |
|---|---|
...
| Known/Prepared/Always Prepared/Ritual/free-cast spells | Implemented per spell; Combat Spell Attacks exposes currently castable tracked spells, slot/Pact/points/free resources, and no-slot ritual casting at ritual speed |
...
| Items, equipment, weapons, attack/damage rolls | Implemented; standard armor and weapon selections populate supported combat and AC fields; damaging spells use mapped SRD dice with upcast/cantrip scaling and custom `XdY` note fallback |
...
| Fantasy quest / choices / skill checks / achievements | Data-driven Adventure mode with an explicit Start Adventure action, three 23-character scene-text rows beside the sprite, branches, rewards, flags, achievements, checkpoints, and 3-second skill-check results |
...
| Device resilience | Heap-fragmentation/peak-allocation host stress tests, allocation stress runner, SD read-only fallback, persistent unsaved warning, failure-only Retry Save action, 9 KB main-app stack, and published hardware matrix |
...
| Cross-FAP navigation | Dolphin Bestiary is a fixed Home entry above Combat and remains launchable after a save failure; Bestiary launches the full Dungeons & Dolphins FAP path; Add to Initiative passes `initiative;Name,HP,AC,InitMod;...`, D&D appends those monsters to the current saved Party Roster, then opens Initiative |
