...
Each non-comment line in `options.txt` uses:
...
Keep IDs stable once published. Run `tools/generate_metadata_pack.sh` after changing the packaged catalogs, then validate the result from Character Builder → Catalog Diagnostics on the Flipper.
