...
static bool pocket_d20_read_character(File* file, PocketSaveData* data) {
    ...
    if(schema < POCKET_D20_OLDEST_SUPPORTED_TEXT_VERSION ||
       !pocket_d20_schema_supported(schema))
        return false;
    if(
...
static bool pocket_d20_migrate_directory(
    ...
    while(storage_dir_read(directory, &info, filename, sizeof(filename))) {
        ...
        int source_length =
            snprintf(source, sizeof(source), "%s/%s", legacy_directory, filename);
...
bool pocket_d20_storage_migrate_legacy_profiles(
    Storage* storage,
    uint16_t* copied_files) {
    furi_assert(storage);
