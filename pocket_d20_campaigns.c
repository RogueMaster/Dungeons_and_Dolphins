...
static bool campaign_copy_file(
    Storage* storage,
    const char* source,
...
static bool campaign_relocate_directory(
    ...
    while(storage_dir_read(directory, &info, filename, sizeof(filename))) {
        ...
        int source_length = snprintf(
            source, sizeof(source), "%s/%s", source_directory, filename);
        int destination_length = snprintf(
            destination, sizeof(destination), "%s/%s", destination_directory, filename);
        ...
        if(file_info_is_dir(&info)) {
            if(!campaign_relocate_directory(
...
static bool campaign_prepare_index_temp(Storage* storage) {
    ...
    bool ok = storage_file_open(
                  file, CAMPAIGN_USER_INDEX_TEMP, FSAM_WRITE, FSOM_CREATE_ALWAYS) &&
...
static bool campaign_merge_legacy_index(Storage* storage, uint16_t* copied_files) {
    ...
    bool ok = storage_file_open(
        source, CAMPAIGN_LEGACY_USER_INDEX, FSAM_READ, FSOM_OPEN_EXISTING);
    char line[CAMPAIGN_LINE_LEN];
    ...
    while(ok && campaign_read_line(source, line, sizeof(line))) {
        ...
        ok = storage_file_open(
                 output, CAMPAIGN_USER_INDEX_TEMP, FSAM_WRITE, FSOM_OPEN_APPEND) &&
    ...
    bool had_index = storage_common_rename(
                         storage, CAMPAIGN_USER_INDEX, CAMPAIGN_USER_INDEX_BACKUP) == FSE_OK;
    if(storage_common_rename(storage, CAMPAIGN_USER_INDEX_TEMP, CAMPAIGN_USER_INDEX) != FSE_OK) {
        if(had_index)
            storage_common_rename(
                storage, CAMPAIGN_USER_INDEX_BACKUP, CAMPAIGN_USER_INDEX);
        storage_common_remove(storage, CAMPAIGN_USER_INDEX_TEMP);
        return false;
    }
    storage_common_remove(storage, CAMPAIGN_USER_INDEX_BACKUP);
...
bool pocket_campaign_migrate_legacy_custom(Storage* storage, uint16_t* copied_files) {
    ...
    while(storage_dir_read(directory, &info, filename, sizeof(filename))) {
        ...
        char destination[256];
        int source_length = snprintf(
            source, sizeof(source), "%s/%s", CAMPAIGN_LEGACY_DIR, filename);
        int destination_length = snprintf(
        ...
        if(file_info_is_dir(&info) && !strncmp(filename, "custom_", 7U)) {
            if(!campaign_relocate_directory(
                   storage, source, destination, 0U, copied_files))
                packs_ok = false;
        } else if(!file_info_is_dir(&info) &&
                  !strncmp(filename, "custom_progress_", 16U)) {
            size_t length = strlen(filename);
            if(length < 5U || strcmp(filename + length - 4U, ".txt") ||
               !campaign_relocate_file_without_replace(
                   storage, source, destination, copied_files))
                progress_ok = false;
        }
    }
