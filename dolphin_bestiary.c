...
static BestiaryApp* bestiary_alloc(void) {
    ...
    bool migration_ok = pocket_monster_migrate_legacy_custom(
        app->storage, &migrated_files);
    bool recovery_ok = migration_ok && pocket_monster_recover_user_pack(
        app->storage, &recovered, &rolled_back);
