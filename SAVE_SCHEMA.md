...
`/ext/apps_data/dndolphins/custom_active_profile.txt` stores `Active=<id>` and is the only companion character-selection metadata. Launch arguments do not override the persisted selection and no companion performs profile-directory discovery for character selection. Journal/Initiative/Bestiary use the lightweight `dnd_profile_ref.c` reader so those FAPs do not need the full storage module solely for metadata. Inventory/Spellbook already link `dnd_storage.c`, so they use its exact active-ID reader and pass that exact ID to the normal character loader; Adventure validates the exact canonical profile through the shared profile-reference helper before loading it. Initiative and Bestiary choose ID `0` only when the active-profile metadata itself is absent or unreadable. If `Active=<id>` is present but that character is missing, Initiative does not switch to 0 or another character; Bestiary keeps the persisted ID and remains usable without a character profile.
